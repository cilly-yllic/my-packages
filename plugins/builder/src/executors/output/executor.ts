...
const runExecutor: PromiseExecutor<OutputExecutorSchema> = async (
  options: OutputExecutorSchema,
  context: ExecutorContext
) => {
  console.log('Executor ran for Output', context.projectName)
  const buildResult = await _runExecutor({ project: context.projectName || '', target: 'build' }, options, context)
